...
class RGBDFrame():
	...
	def load(self, file_handle):
		...
		# self.color_data = bytes(struct.unpack('c'*self.color_size_bytes, file_handle.read(self.color_size_bytes)))
...
class SensorData:
	...
	def load(self, filename):
		with open(filename, 'rb') as f:
			...
			# print(self.color_compression_type)
			# print(self.depth_compression_type)
	...
	def export_color_images(self, output_path, image_size=None):
		...
		# 	cv2.imwrite(os.path.join(output_path, str(f) + '.jpg'), color)
		for i, f in enumerate(range(0, self.num_frames, self.frame_skip)):
			cv2.imwrite(os.path.join(output_path, str(f) + '.jpg'), self.color_images[i])
